...
//		add_action( 'admin_post_nopriv_pixelgradelt_conductor_check_authorization_header', [ $this, 'handle_authorization_request' ] );
	}
	...
	protected static function display_notice( $message ) {
		printf(
			...
			esc_html__( 'Health Check', 'pixelgradelt_conductor' ),
			wp_kses(
				$message,
				[
					'a' => [
	...
	public static function display_permalink_notice() {
		...
		$message = sprintf(
			...
			__( 'PixelgradeLT Conductor requires pretty permalinks to be enabled. <a href="%s">Enable permalinks</a>.', 'pixelgradelt_conductor' ),
	...
	public static function check_authorization_header() {
		$url = add_query_arg(
			[
				'action' => 'pixelgradelt_conductor_check_authorization_header',
			],
		...
		$response = wp_remote_get(
			...
			[
				'headers'   => [
					...
					'Authorization' => 'Basic ' . base64_encode( '%api_key%:pixelgradelt_conductor' ),
				],
				'timeout'   => 10,
	...
	public function handle_authorization_request() {
		...
		if ( empty( $header ) ) {
			$this->send_json_error(
				...
				sprintf(
					...
					'https://github.com/pixelgradelt/pixelgradelt-conductor/blob/develop/docs/troubleshooting.md#basic-auth-not-working'
				)
			);
		}
		...
		if ( empty( $password ) || 'pixelgradelt_conductor' !== $password ) {
			$this->send_json_error(
				'invalid_password',
