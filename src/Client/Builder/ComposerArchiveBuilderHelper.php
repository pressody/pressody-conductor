...
class ComposerArchiveBuilderHelper
{
    ...
    public function __construct(IOInterface $output, array $archiveConfig)
    {
        $this->output = $output;
    ...
    public function isSkippable(PackageInterface $package): bool
    {
        ...
        if (true === $this->archiveConfig['skip-dev'] && true === $package->isDev()) {
            $this->output->write(sprintf("<info>Skipping '%s' (is dev)</info>", $name));
        ...
        if ($this->archiveConfig['whitelist'] && !$this->isOneOfNamesInList($names, $this->archiveConfig['whitelist'])) {
            $this->output->write(sprintf("<info>Skipping '%s' (is not in whitelist)</info>", $name));

            return true;
        }

        if ($this->archiveConfig['blacklist'] && $this->isOneOfNamesInList($names, $this->archiveConfig['blacklist'])) {
            $this->output->write(sprintf("<info>Skipping '%s' (is in blacklist)</info>", $name));
